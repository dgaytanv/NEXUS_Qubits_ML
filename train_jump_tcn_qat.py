#!/usr/bin/env python3
"""
Quantization-Aware Training (QAT) for the true TCN jump detector.

Loads pretrained float32 weights from train_jump_tcn.py, rebuilds the
architecture with QKeras quantized layers (ap_fixed<16,6>), transfers
weights, and fine-tunes.

Architecture (matches train_jump_tcn.py):
    Input(128, 2)
    → TCN Block 0: 2x QConv1D(8,  k=3, dil=1, causal) + residual
    → TCN Block 1: 2x QConv1D(16, k=3, dil=2, causal) + residual
    → AveragePooling1D(128) → Flatten → QDense(1) → Sigmoid

Changes from float32 version:
    - Conv1D       → QConv1D      (quantized_bits(16,6))
    - Dense        → QDense       (quantized_bits(16,6))
    - 1x1 residual Conv1D → QConv1D (quantized_bits(16,6))
    - ReLU kept as standard (QKeras quantized_relu added after BN)
    - GlobalMaxPool → AveragePooling1D(128) + Flatten (hls4ml compat)

Usage:
    python train_jump_tcn_qat.py \
        --data-dir /home/export/jgaytanv/quantum/Data_Output_100k/flat \
        --pretrained training_output_tcn_100k/best_model.h5 \
        --output-dir training_output_tcn_qat
"""
...
# ── QAT TCN building blocks ──

def qat_residual_block(x, filters, kernel_size, dilation, block_idx):
    qb = quantized_bits(bits=TOTAL_BITS, integer=INT_BITS, alpha=1)
    residual = x

    x = QConv1D(filters, kernel_size, dilation_rate=dilation, padding="causal",
                use_bias=True, kernel_initializer="he_normal",
                kernel_quantizer=qb, bias_quantizer=qb,
                name=f"tcn_conv_a_{block_idx}")(x)
    x = layers.ReLU(name=f"tcn_relu_a_{block_idx}")(x)
    x = layers.BatchNormalization(name=f"tcn_bn_a_{block_idx}")(x)
    #x = QActivation(f"quantized_relu(bits={TOTAL_BITS}, integer={INT_BITS})",
    #                 name=f"tcn_qclip_a_{block_idx}")(x)      # <-- added
    x = QActivation(quantized_bits(bits=TOTAL_BITS, integer=INT_BITS, alpha=1),
                     name=f"tcn_qclip_a_{block_idx}")(x)

    x = QConv1D(filters, kernel_size, dilation_rate=dilation, padding="causal",
                use_bias=True, kernel_initializer="he_normal",
                kernel_quantizer=qb, bias_quantizer=qb,
                name=f"tcn_conv_b_{block_idx}")(x)
    x = layers.ReLU(name=f"tcn_relu_b_{block_idx}")(x)
    x = layers.BatchNormalization(name=f"tcn_bn_b_{block_idx}")(x)
    #x = QActivation(f"quantized_relu(bits={TOTAL_BITS}, integer={INT_BITS})",
    #                 name=f"tcn_qclip_b_{block_idx}")(x)      # <-- added
    x = QActivation(quantized_bits(bits=TOTAL_BITS, integer=INT_BITS, alpha=1),
                     name=f"tcn_qclip_b_{block_idx}")(x)

    if residual.shape[-1] != filters:
        residual = QConv1D(filters, 1, use_bias=False,
                           kernel_initializer="he_normal",
                           kernel_quantizer=qb,
                           name=f"tcn_res_{block_idx}")(residual)

    x = layers.Add(name=f"tcn_add_{block_idx}")([x, residual])
    return x

def build_qat_model():
    """Build the QKeras quantized TCN."""
    qb = quantized_bits(bits=TOTAL_BITS, integer=INT_BITS, alpha=1)
    ...
    for d in range(DEPTH):
        ...
        x = qat_residual_block(x, filters, KERNEL_SIZE, dilation, block_idx=d)

    # hls4ml-compatible pooling
    ...
    model = keras.Model(inputs=inp, outputs=x, name="jump_tcn_qat")
    return model


# ── Float32 model (for loading pretrained weights) ──

def float_residual_block(x, filters, kernel_size, dilation, block_idx):
    residual = x
    x = layers.Conv1D(filters, kernel_size, dilation_rate=dilation, padding="causal",
                      use_bias=True, kernel_initializer="he_normal",
                      name=f"tcn_conv_a_{block_idx}")(x)
    x = layers.ReLU(name=f"tcn_relu_a_{block_idx}")(x)
    x = layers.BatchNormalization(name=f"tcn_bn_a_{block_idx}")(x)
    ...
    x = layers.BatchNormalization(name=f"tcn_bn_b_{block_idx}")(x)
    if residual.shape[-1] != filters:
        residual = layers.Conv1D(filters, 1, use_bias=False,
                                 kernel_initializer="he_normal",
                                 name=f"tcn_res_{block_idx}")(residual)
    x = layers.Add(name=f"tcn_add_{block_idx}")([x, residual])
    return x
...
def build_float_model():
    ...
    for d in range(DEPTH):
        filters  = min(BASE_FILTERS * (2 ** d), 256)
        dilation = BASE_DILATION ** d
        x = float_residual_block(x, filters, KERNEL_SIZE, dilation, block_idx=d)
    x = layers.GlobalMaxPooling1D(name="global_maxpool")(x)
    x = layers.Dense(1, name="dense_out")(x)
    x = layers.Activation("sigmoid", name="sigmoid")(x)
    return keras.Model(inputs=inp, outputs=x, name="jump_tcn_c15_true")


def transfer_weights(float_model, qat_model):
    """Transfer weights by name — Conv1D/QConv1D share weight names."""
...
# ── Data loading (same as train_jump_tcn.py) ──
...
def save_results_log(output_dir, args, model, history,
                     ...
                     qubit_results, float_test_acc, float_efficiency):
    import datetime
    ...
    with open(os.path.join(output_dir, "results.txt"), "w") as f:
        def w(line=""):
            f.write(line + "\n")
        w("=" * 65)
        w("QAT RESULTS — True TCN (train_jump_tcn_qat.py)")
        w("=" * 65)
        ...
        for d in range(DEPTH):
            f_ = min(BASE_FILTERS * (2 ** d), 256)
            dil = BASE_DILATION ** d
            w(f"  TCN Block {d}: 2x QConv1D(f={f_}, k={KERNEL_SIZE}, dil={dil}) + residual")
        ...
        w()
        w("── Accuracy degradation from quantization ─────────────────────")
        ...
        w()
        w("── FPGA deployment ──────────────────────────────────────────")
        w(f"  Target precision: ap_fixed<{TOTAL_BITS},{INT_BITS}>")
        w(f"  Strategy:         Resource / io_stream")
        w(f"  Next: convert with hls4ml, synthesize on xwing")
        w("=" * 65)
...
def main():
    parser = argparse.ArgumentParser(description="QAT for TCN jump detector")
    parser.add_argument("--data-dir",    type=str, required=True)
    parser.add_argument("--pretrained",  type=str, required=True,
                        help="Path to float32 best_model.h5")
    parser.add_argument("--epochs",      type=int,   default=100)
    parser.add_argument("--batch-size",  type=int,   default=256)
    parser.add_argument("--lr",          type=float, default=1e-4)
    parser.add_argument("--output-dir",  type=str,   default="training_output_tcn_qat")
    ...
    float_prob = float_model.predict(X_test, verbose=0).flatten()
    float_pred = (float_prob > 0.5).astype(int)
    ...
    pre_loss, pre_acc = qat_model.evaluate(X_test, y_test, verbose=0)
    ...
    save_results_log(args.output_dir, args, qat_model, history,
                     test_loss, test_acc, y_test, y_pred, y_prob,
                     qubit_results, float_acc, float_eff)

    qat_model.save(os.path.join(args.output_dir, "jump_tcn_qat.h5"))
    print(f"\nQAT model saved to {args.output_dir}/jump_tcn_qat.h5")

    print("\n" + "=" * 65)
    print("NEXT STEPS:")
    print(f"  1. Convert to HLS:")
    print(f"       python convert_to_hls.py --model {args.output_dir}/jump_tcn_qat.h5")
    print(f"  2. Synthesize on xwing (needs Vitis 2025.2)")
    print(f"  3. Integrate with QICK firmware")
    print("=" * 65)
